...
// Adding the file renaming functionality
// const fs = require('fs');
...
// Function to rename files with UUID
function renameFilesWithUUID() {
    const uploadsDir = './public/uploads/';
    
    // Ensure the directory exists
    if (!fs.existsSync(uploadsDir)) {
        console.error(`Directory ${uploadsDir} does not exist!`);
        return;
    }
    ...
    files.forEach(fileName => {
        ...
        if (fs.existsSync(filePath)) {
            try {
                ...
                // Parse the filename to insert UUID before extension
                const fileExtension = path.extname(fileName);
                const fileNameWithoutExt = path.basename(fileName, fileExtension);
                const newFileName = `${fileNameWithoutExt}_${uuid}${fileExtension}`;
...
// Function to reverse UUID renaming using the original files list
function reverseUUIDRenaming() {
    const uploadsDir = './public/uploads/';
    ...
    // UUID pattern to match in filenames
    const uuidPattern = /^(.+)_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(\..+)$/i;
    
    // Create a map of filename prefixes to their original complete filenames
    const originalFilenamesMap = {};
    files.forEach(originalFilename => {
        const fileExtension = path.extname(originalFilename);
        const filenameWithoutExt = path.basename(originalFilename, fileExtension);
        originalFilenamesMap[filenameWithoutExt.toLowerCase()] = originalFilename;
    });
    ...
    filesInDir.forEach(fileName => {
        ...
        if (match) {
            try {
                // Extract the filename part before the UUID
                const fileNamePrefix = match[1];
                
                // Look up the original filename in our map (case insensitive)
                const originalFileName = originalFilenamesMap[fileNamePrefix.toLowerCase()];
                
                if (originalFileName) {
                    // Define paths
                    const currentPath = path.join(uploadsDir, fileName);
                    const newPath = path.join(uploadsDir, originalFileName);
                    
                    // Rename file back to original name
                    fs.renameSync(currentPath, newPath);
                    
                    console.log(`Reversed renaming: ${fileName} -> ${originalFileName}`);
                    renamedCount++;
                } else {
                    console.warn(`Could not find original filename for: ${fileName}`);
                    skippedCount++;
                }
            } catch (err) {
                console.error(`Error reversing rename for ${fileName}: ${err.message}`);
                errorCount++;
            }
        } else {
            // File doesn't match the UUID pattern, so skip it
            skippedCount++;
        }
    });
    
    console.log(`\nReverse rename operation completed:`);
    console.log(`- Files successfully restored to original names: ${renamedCount}`);
    console.log(`- Files skipped (no UUID or no match in original list): ${skippedCount}`);
    console.log(`- Files with errors: ${errorCount}`);
}
...
// If you want to reverse the renaming, uncomment this line:
reverseUUIDRenaming();